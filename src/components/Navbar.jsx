    ...
    <div>
      <div className="flex justify-between items-center px-4 md:px-8 py-4 bg-slate-900 text-white z-50 w-full fixed">
        <div className="flex items-center">
          <Link to="/">
            <img src={desoc || "/placeholder.svg"} alt="Logo" className="h-16 md:h-25 -my-8 md:-my-17 z-10" />
          </Link>
        </div>
        ...
        <div className="hidden md:block">
          <ul className="flex gap-8 md:gap-24 list-none text-lg md:text-2xl pr-5">
            <li>
              <Link
                to="/"
                className="text-white no-underline font-medium transition-colors duration-300 hover:text-sky-400"
              >
                Home
              </Link>
            </li>
      ...
      <div className={`fixed inset-0 z-40 md:hidden ${isMobileMenuOpen ? "block" : "hidden"}`}>
        ...
        {/* Sidebar */}
        <div
          className={`fixed right-0 top-0 h-full w-64 bg-slate-900 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
        >
